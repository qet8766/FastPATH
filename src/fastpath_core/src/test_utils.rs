...
/// Create a `CompressedTileData` from the test JPEG bytes.
pub fn test_compressed_tile() -> CompressedTileData {
    CompressedTileData {
        jpeg_bytes: Bytes::from(test_jpeg_bytes()),
        width: 1,
        height: 1,
    }
}
...
pub fn create_test_fastpath(dir: &Path) {
    let metadata = r#"{
...
pub fn create_test_fastpath_with_tiles(dir: &Path) {
    let metadata = r#"{
...
pub fn compute_test_slide_id(dir: &Path) -> u64 {
